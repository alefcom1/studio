...
### 5.3 Перевод сайта по ссылке (AI, до 5 страниц)
...
Воронка (подтверждена владельцем): бесплатный перевод текста по ссылке →
CTA: «Ещё страницы (ИИ)» → Stars; «Заказать локализацию под ключ — с
полным форматированием страниц, публикацией и SEO» → короткий бриф →
кабинет (контур B, §6.3), результат AI-перевода прикладывается к брифу.
...
### 6.3 Маршрутизация заказов (решение владельца, 20.07.2026)

Два производственных контура — по типу работы:

**Контур A — профессиональный перевод документов → TMS (tms.perevod4.ru).**
После бесплатной оценки (§5.2) заказ уходит в TMS:
`POST /public/upload` → `POST /public/widget-orders` (контракт tms.js:
X-API-Key, contact, language_pair, service_type, file_ids, urgency,
estimated_price) → номер заказа клиенту; статусы — `GET /public/orders/{id}`
(поллинг из worker, уведомления в бота); готовый перевод —
`GET /public/orders/{id}/result` кнопкой в «Моих проектах».

**Контур B — разработка сайтов/приложений и ВСЯ локализация сайтов →
кабинет remarka.biz (cab.remarka.biz).**
(Решение владельца 20.07.2026: локализация целиком в кабинет — помимо
перевода текста там почти всегда техработы: форматирование страниц,
hreflang, CMS, публикация, переключатель. TMS остаётся чисто
документо-переводческим контуром.)
Полный бриф в Telegram (мастер §5.4/§7 — те же вопросы, что собирает
прототип, плюс бюджетная вилка и сроки) → в конце запрашиваем e-mail:
- e-mail дан → self-serve создание CabUser (open signup уже работает в
  кабинете) + заявка становится проектом в staff-области кабинета с
  приложенным брифом (Json) и ссылкой на Telegram-профиль; клиент получает
  magic-ссылку в кабинет, где дальше живут этапы/файлы/счета (CabInvoice —
  уже готовый модуль);
- e-mail не дан → остаётся `MktLead` с полным брифом, владелец дожимает
  вручную из админки.
Для локализации бриф короче девелоперского: URL, целевые языки, CMS,
нужна ли публикация; к брифу автоматически прикладывается результат
бесплатной AI-локализации §5.3, если она была (готовый контекст для
оценки).

- Lead scoring (упрощённый): +балл за файл, скан сайта, выбор языков,
  дохождение до оплаты, повторное открытие результата, оставленный e-mail.
  Порог «горячего» — в настройках; горячие — мгновенно владельцу в Telegram
  (ADMIN_CHAT_ID).
...
Три полных языка: RU / EN / IT. Автоопределение решает 90% случаев:
`Telegram.WebApp initData language_code` (it→IT, en→EN, ru→RU) →
кука `remarka_lang` (если пришёл с наших сайтов) → **дефолт из настроек**
(`MktSetting market.default_locale`).

Дефолт на старте — **русский**: трафик первой волны идёт из Telegram-каналов
русскоязычной аудитории (RU-страницы remarka.biz, аудитория 1russian.com,
диаспорные чаты); итальянец с итальянским Telegram всё равно автоматически
получит итальянский. Дефолт НЕ хардкодится — когда появится итальянский
трафик (Telegram Ads IT, WhatsApp-канал), переключается из админки без
деплоя.

Паттерн Monitor: RU-строка = ключ, `dict.ts` `{en, it}`, `useT()`. Ручной
переключатель в профиле, выбор сохраняется в `MktUser.locale` и имеет
приоритет над автоопределением. Контент каталога — из `MktProductI18n`
(админка), не из словаря.
...
## 12-bis. AI-шлюз и экономия токенов (решение владельца, 20.07.2026)

Ключ — существующий аккаунт Anthropic владельца (тот же, что уже питает
AI-инструменты сайта и рекомендации Monitor). В консоли Anthropic завести
**отдельные ключи на потребителя** (wp-tools / monitor / market) — биллинг
единый, расходы видны раздельно.

Шлюз — НЕ в WordPress, а в платформе: расширяем `packages/integrations`
(ClaudeClient уже там, с прокси-режимом) до модуля `ai-gateway`, которым
пользуются worker-джобы Market. WP-инструменты сайта уже имеют свой
эквивалент (`remarka_ai_call()` с классами fast/smart и раздельными
дневными капами — паттерн подтверждён в проде) и остаются как есть;
перевод их на общий шлюз — опциональный шаг после M4.

Маршрутизация моделей (тот же принцип fast/smart, что в теме сайта):

| Класс | Модель | Задачи Market |
|---|---|---|
| fast | `claude-haiku-4-5` | определение языка, классификация тематики документа, извлечение терминов, QA-подсветка, подсчёт/анализ структуры, hero-извлечение |
| smart | `claude-sonnet-5` | переводы (фрагмент и полный), локализация страниц, концепция+sitemap прототипа, контент макета, Blueprint |
| top | — | в бесплатных инструментах НЕ используется; включается точечно решением владельца |

Экономия токенов (обязательно к реализации в шлюзе):
1. **Prompt caching** — системные промпты инструментов длинные и
   постоянные; кэширование срезает их стоимость на повторных вызовах.
2. **Batch API (−50%)** — все асинхронные джобы (локализация 5 страниц,
   полный AI-перевод документа, Blueprint) и так идут через очередь без
   требования мгновенного ответа — гонять их батчами.
3. **Кэш результатов по хэшу входа** — повторный запуск того же
   файла/URL/текста тем же пользователем в течение N дней отдаёт
   сохранённый результат из `MktToolRun` бесплатно.
4. Жёсткие `max_tokens` на каждый вызов; входные лимиты знаков/страниц.
5. Учёт: `MktToolRun.costUsd` + дневные капы стоимости per-инструмент и
   общий (`MktSetting`), при исчерпании — честный отказ до завтра.
...
5. Завести в консоли Anthropic отдельный API-ключ «market» (тот же аккаунт,
   раздельная видимость расходов) — нужен к началу M1.
