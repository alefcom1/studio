...
**Контур A — переводы и текстовая локализация → TMS (tms.perevod4.ru).**
...
`GET /public/orders/{id}/result` кнопкой в «Моих проектах». Сюда же —
локализация «только тексты» (перевод контента страниц без техработ).

**Контур B — разработка сайтов/приложений и локализация под ключ →
...
- e-mail не дан → остаётся `MktLead` с полным брифом, владелец дожимает
  вручную из админки.
Критерий выбора контура для локализации: есть техработы (hreflang, CMS,
публикация, переключатель) → контур B; чисто перевод контента → контур A.
