...
> Дата: 18.07.2026. Статус: **реализовано** (та же дата). Решения владельца:
> (1) на lab.remarka.biz будет полноценная авторизация, публичная витрина —
> позже; пока на сайте размещена только ссылка на платформу; (2) K2 кабинета
> добирается до прода → копирайт опубликован в полной версии, включая
> [K2]-строки (согласования/файлы/счета/вопросы); (3) Monitor подаётся как
> часть поддержки студии, без публичной цены.
> Реализация: страница area-clienti ×3 языка, промо-блок на главных ×3,
> секция Gratis/Monitor (#monitor) на индексах инструментов ×3, строка в
> услугах ×3 (включая RU-лендинги), строка в таблицах prezzi ×3, абзац в
> chi-siamo ×3, футер ×3, hreflang-тройка, SEO-мета в seo-meta.md. Основание: `piano-cabinet-k1.md`,
...
## Пост-запуск: обратная связь Product Hunt (19.07.2026, реализовано)

По отзывам запуска реализованы: (1) блок реальных отзывов «Dalla community»
на /strumenti/ и странице check-up ×3 языка (цитаты в оригинале, разрешение
авторов, без schema.org Review — self-serving); (2) блок приоритетов «Da dove
partire» поднят ПЕРЕД семью светофорами; (3) кнопка «Scaricate il PDF adesso»
— мгновенное скачивание отчёта без e-mail (эндпоинт
`remarka_tool_report_download`, лимит 3/час/IP, без сбора данных);
(4) дельта «rispetto all'ultima scansione» в localStorage посетителя + мостик
на Monitor (JS, без сервера); (5) микро-фидбек «Vi è stato utile?» на всех
инструментах (JS-инъекция, эндпоинт `remarka_tool_feedback`, голос+комментарий
на почту студии, без персональных данных); (6) нативные комментарии WP
открыты ТОЛЬКО на статьях блога (deploy-import: comment_status=open +
полная премодерация, opt-in cookies). В бэклоге: сравнение с конкурентом
(2 URL) — после наблюдения за квотой PSI.
⚠️ Проверить после деплоя: выводит ли шаблон страницы Prespa comments_template()
на статьях блога — если нет, комментарии включены, но форма не отрисуется
(понадобится маленький template-хук в теме).
