...
## Приложение: рубрика замера (Фаза 4)

Ежемесячно в Google Search Console (Италия, поиск IT). Цель — один URL на
один запрос, рост позиций pillar, работающая перелинковка.

**A. Каннибализация (главное).** Performance → Queries. Для каждого
канонического запроса pillar (см. §2 и фокус-ключи в `seo-meta.md`) открыть
запрос → вкладка Pages. Норма = 1 URL. Если по запросу мигают 2 URL:
- определить, кто должен ранжироваться (обычно денежная страница),
- на «лишней» странице ослабить ключ в Title/H1 и добавить ссылку на pillar
  с точным анкором,
- перепроверить через 2–4 недели.

**B. Позиции pillar.** Для 11 pillar-запросов вести таблицу «запрос →
позиция → CTR → показы» помесячно. Растущая позиция + растущий CTR = кластер
работает. Падение CTR при хорошей позиции → переписать Title/Description.

**C. Внутренние переходы.** В аналитике смотреть переходы pillar↔articolo
(события клика по блоку «Approfondimenti» и по ссылкам «Leggi anche»).
Нулевые клики по блоку кластера → пересмотреть анкоры/позицию блока.

**D. Покрытие кластеров.** Раз в квартал: у каждого pillar ≥3 живых
спутника? Дыры (§3: e-commerce, PWA, гео) закрывать статьями в очередь блога.

**E. Быстрый само-чек каннибализации по репозиторию** (до публикации новой
страницы): греп фокус-ключа по `docs/seo-meta.md` и заголовка по
`build-tools/data.py` — если ключ уже занят, выбрать другой long-tail.
