...
- **Risultato (с цифрой):** `[ЦИФРА ВЛАДЕЛЬЦА — НЕ ПОЛУЧЕНА 15.07: partite giocate / tempo medio sul sito con il gioco]` —
  un contenuto che trattiene e fa parlare del marchio. *(единственный кейс без цифры — публиковать вариант без цифр)*
...
- **Result (with number):** `[OWNER NUMBER — NOT PROVIDED 15.07: games played / average time on site with the game]` —
  content that holds attention and gets the brand talked about. *(the only case without a number — publish the no-number variant)*
...
- **Текст:**
  ...
  > посетитель остался. А для гостевого дома **ukrinitsy.ru** — простой быстрый сайт-визитку:
...
| **Gioco** | **цифры НЕ получены** — плейсхолдер оставлен, публикуется вариант без цифр | — |
