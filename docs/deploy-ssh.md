...
**M5 (обогащение отчёта: графики, находки, полный логотип) — деплой только
темы.** Этап M5 меняет исключительно файлы темы: `assets/js/remarka.js` (сбор
`payload.findings` — до 4 аудитов на измерение × до 3 ресурсов, капы на
клиенте), `functions.php` (`remarka_tool_report_sanitize_findings()` —
повторная санитизация на сервере, whitelist структуры, `esc_url_raw`
http/https, `wp_strip_all_tags` + `mb_substr` на каждую строку) и
`inc/checkup-report-pdf.php` (обложка: логотип `assets/img/logo-full.png`
как base64 data-URI — обходит `chroot` dompdf, не читается с диска — и
SVG/GD-кольцо композита; страница-резюме: горизонтальная диаграмма 7
измерений; каждый раздел измерения: полоса балла с отметками 90/75/50 +
для CO₂ отдельная сравнительная шкала против медианы 0,8 г + блок «Cosa
abbiamo trovato su questa pagina» с кликабельными ссылками из `findings`).
**Контент (`patterns/**`, БД) не тронут** — полный деплой-цикл
`REMARKA_FORCE=1 wp eval-file deploy-import.php` для M5 не нужен, достаточно
обычной синхронизации файлов темы (rsync/scp `wordpress/remarka-studio/` на
сервер + сброс opcache при необходимости). Обратная совместимость: если
клиент прислал payload без `findings` (старый JS в кэше браузера), сервер
подставляет пустые массивы по всем 7 ключам и PDF рендерится без секций
находок — валиден в любом случае.
