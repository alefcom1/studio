...
| **M5** | Sonnet | Обогащение PDF по трём решениям владельца: графики в каждом разделе, индивидуальные находки со ссылками на ресурсы, полный логотип на обложке. |
...
- **M5 — completato (15.07.2026).** Arricchimento del report PDF sulle 3
  decisioni del titolare: grafici in ogni sezione, rilievi individuali con
  link alle risorse concrete, logo pieno in copertina. **Client
  (`remarka.js`):** `psiFetch` estende `topAudits()` a `perf`/`bp` (prima
  solo `a11y`/`seo`) e a ogni audit aggiunge fino a 3 risorse concrete da
  `details.items` (`{u:url, v:valore leggibile — KB/ms}`); `runGdprCheck`
  espone `domains` (hostname di terze parti già intercettati dalla regex
  esistente); nuovo `buildCheckupFindings()` assembla
  `payload.findings = {perf,seo,a11y,gdpr,bp,ai,co2}` con cap lato client
  (≤4 gruppi/dimensione, ≤3 item/gruppo, stringhe ≤200/url ≤300 caratteri,
  `capFindingsSize()` degrada gli item se il totale supera ~40KB) — raccolta
  silenziosa, lo schermo dei risultati non cambia (verificato via
  Playwright). **Server (`functions.php`):** nuova
  `remarka_tool_report_sanitize_findings()` — whitelist delle 7 chiavi,
  stessi cap del client riverificati (il server non si fida mai del
  client), ogni stringa `wp_strip_all_tags()` + `mb_substr()`, ogni url
  `esc_url_raw()` solo http/https (uno scarto silenzioso dell'url, non
  dell'intero item, se il testo resta valido). **PDF
  (`inc/checkup-report-pdf.php`):** copertina con logo pieno
  (`assets/img/logo-full.png` incorporato come data-URI base64 — il chroot
  di dompdf punta a `lib/dompdf/`, un `<img src="…png">` esterno non
  verrebbe letto, il data-URI aggira il filesystem) + anello (donut) del
  composito colorato sul verdetto, renderizzato **in GD** (non SVG: nei
  test l'SVG inline via data-URI non si centrava in modo affidabile con
  dompdf/php-svg-lib, un PNG raster centra esattamente come il logo);
  pagina Riepilogo con diagramma orizzontale delle 7 dimensioni
  (`remarka_checkup_pdf_chart()`, barre CSS); ogni pagina-dimensione con
  barra "il vostro punteggio" e tacche alle soglie reali 90/75/50 (mai una
  finta media di mercato) + per CO₂ una scala comparativa contro la
  mediana dichiarata (0,8 g, modello SWD) — i grammi sono l'inversa esatta
  della rampa già dichiarata in §1.2, non un dato nuovo; blocco «Cosa
  abbiamo trovato su questa pagina» con link cliccabili ai `findings`
  sanificati + link alla pagina analizzata, che sparisce senza rompere il
  PDF se `findings` è vuoto/assente (client vecchio). Oggettività: nuova
  riga «Analizziamo la pagina che ci avete indicato, non l'intero sito: un
  audit completo, pagina per pagina, è uno dei nostri servizi a pagamento»
  su pagina Riepilogo e pagina Metodologia, 3 lingue. Verifica: `php -l` su
  `functions.php`/`inc/checkup-report-pdf.php`, `node -e new Function()` su
  `remarka.js`, `git diff --stat` conferma solo i 3 file attesi. Harness
  Node/PHP esteso (`scratchpad/test_checkup_report_m5.php`, 95 controlli):
  tutti i controlli M3 + nuovi su findings (struttura pulita, XSS in
  `t`/`u`/`v` — url `javascript:` scartato ma item con solo testo
  sopravvive, cap gruppi/item/lunghezza stringhe, strutture malformate
  senza fatal, findings assenti → PDF valido senza sezione, logo
  incorporato come `/Subtype/Image` nel flusso PDF, inversione grammi CO₂)
  — tutti verdi. 3 PDF di esempio con rilievi demo ricchi (2 script che
  rallentano con URL, un'immagine pesante, 3 domini tracker, llms.txt
  mancante) in `scratchpad/m5_report_it.pdf`/`_en.pdf`/`_ru.pdf`
  (~392–403 KB l'uno, più pesanti dei report M3 per il logo incorporato),
  verificati pagina per pagina via rendering PNG (PyMuPDF, non disponibili
  poppler/imagemagick in questa sessione). Regressione widget: Playwright
  (Chromium `/opt/pw-browsers/chromium`) sulla pagina check-up IT con
  `fetch` mockato (fixture PSI ricca con `details.items`, HTML con 3
  tracker, llms.txt/robots/sitemap) a 390 e 1440px — schermo risultati
